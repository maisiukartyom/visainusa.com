    ...
    useEffect(() => {
        const verifyCookie = async (level) => {
           try{
            ...
            navigate('/');
            toast.error("You don't have level 2!", {
                position: "top-center",
                autoClose: 6000,
     ...
     const sendContract = async () => {
        try{
            await axios.post('/email/sendContract', {email: user.email, phoneNumber: user.phoneNumber, fullname: user.name});
            toast.success("The contract will be sent to your email shortly!", {
                position: "top-center",
                autoClose: 6000,
                hideProgressBar: false,
                closeOnClick: true,
...
<iframe    src="https://www.youtube.com/embed/ZOAMcjBvYCo?si=IiHAwWdsFiOXJg2C" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay;
 clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share; fullscreen" allowfullscreen className="youtube-levelApply"></iframe> 
                ...
                <button className="btn-apply" onClick={sendContract}>SEND ME CONTRACT</button>
                </div>
                </div>
